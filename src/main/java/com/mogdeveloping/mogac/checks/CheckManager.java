...
public class CheckManager {
    ...
    private void registerChecks() {
        ...
        checks.add(new com.mogdeveloping.mogac.checks.movement.Fly(plugin));
        applyConfig();
    }

    private void applyConfig() {
        for (Check check : checks) {
            String path = "checks." + check.getName() + ".";
            if (plugin.getConfig().contains(path + "enabled")) {
                check.setEnabled(plugin.getConfig().getBoolean(path + "enabled"));
            }
            if (plugin.getConfig().contains(path + "max-violations")) {
                check.setMaxViolations(plugin.getConfig().getInt(path + "max-violations"));
            }
            if (plugin.getConfig().contains("punish-command")) {
                check.setPunishCommand(plugin.getConfig().getString("punish-command"));
            }
        }
    }
